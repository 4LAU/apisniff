...
| `spec -f` | `json` gives JSON; **every other value silently gives YAML**, including a typo. `-f jsno -o spec.json` exits 0 and writes YAML into a `.json` file. `--json` is a safer shorthand — it cannot be misspelled into silent YAML |
| `spec --json` | Same as `-f json`. The two are mutually exclusive: passing both fails |
| `--impersonate` | `chrome` or `firefox` on both `probe` and `replay`; any other value is rejected |
